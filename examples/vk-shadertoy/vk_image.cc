...
void VulkanImage::Create() {
  ...
  if (will_be_initialized_ || host_visible_) {
    usage_ = static_cast<VkImageUsageFlagBits>(
        usage_ &
    ...
    res = d_.vkGetPhysicalDeviceImageFormatProperties(
        physical_.device, format_, VK_IMAGE_TYPE_2D, tiling, usage_, 0,
        &format_properties);
    if (res != VK_SUCCESS) {
  ...
  if (mipmaps_) {
    mipLevels = static_cast<uint32_t>(
        (log(std::max(extent_.width, extent_.height)) / log(2)) + 1);
  }
  ...
  uint32_t mem_index =
      find_memory(physical_.memory_properties, mem_req,
  ...
  if ((usage_ & VK_IMAGE_USAGE_SAMPLED_BIT)) {
    ...
    sampler_info.anisotropyEnable =
        anisotropyEnable_ && physical_.features.samplerAnisotropy;
    sampler_info.maxAnisotropy =
        physical_.properties.limits.maxSamplerAnisotropy;
