...
static vk_error
init_texture_mem(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_render_essentials *essentials,
                 struct vk_image *image, uint8_t *texture, int width, int height, const char *name, bool mipmaps,
                 bool linear) {
    ...
    *image = (struct vk_image) {
    ...
    retval = VulkanRender::init_texture(phy_dev, dev, essentials, image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                        texture,
                                        name);
    return retval;
}
...
static vk_error
init_texture_file(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_render_essentials *essentials,
...
static vk_error
texture_empty(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_render_essentials *essentials,
              struct vk_image *image, int width, int height) {
    vk_error retval = VK_ERROR_NONE;
    ...
    for (unsigned int i = 0; i < height; ++i) {
        for (unsigned int j = 0; j < width; ++j) {
            size_t pixel = (i * static_cast<unsigned int>(width) + j) * 4 * sizeof(uint8_t);
