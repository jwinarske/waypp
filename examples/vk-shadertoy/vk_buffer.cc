...
VulkanBuffer::VulkanBuffer(
    ...
    : d_(dispatcher),
      device_(device),
      phy_dev_mem_props_(phy_dev_mem_props),
      size_(size),
      usage_(usage),
      host_visible_(false) {}
...
vk::Result VulkanBuffer::create() {
  ...
  uint32_t mem_index =
      find_memory(phy_dev_mem_props_, mem_req,
  ...
  CHECK_VK_RESULT(
      d_.vkAllocateMemory(device_, &mem_info, nullptr, &buffer_mem_));
  ...
  if (make_view_) {
    if ((usage_ & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) ||
      ...
      CHECK_VK_RESULT(
          d_.vkCreateBufferView(device_, &view_info, nullptr, &view_));
    }
  }
