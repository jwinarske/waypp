/*
 * @copyright Copyright (c) 2024 Joel Winarske
...
uint32_t EventTimer::watched_fd_ = 0;
int EventTimer::evfd_ = -1;
...
EventTimer::EventTimer(int clock, evtimer_cb callback, void *callback_data)
    ...
    if (evfd_ < 0) {
        ...
        evfd_ = epoll_create1(EPOLL_CLOEXEC);
        if (evfd_ < 0) {
            if (errno != EINVAL) {
    ...
    timerfd_ = timerfd_create(clock, TFD_CLOEXEC | TFD_NONBLOCK);
    if (timerfd_ == -1) {
...
void EventTimer::set_timerspec(int32_t rate, int32_t delay) {
    ...
    timerspec_.it_interval.tv_sec = repeat_rate_sec;
    timerspec_.it_interval.tv_nsec = repeat_rate_nsec;
    timerspec_.it_value.tv_sec = repeat_delay_sec;
    timerspec_.it_value.tv_nsec = repeat_delay_nsec;

    SPDLOG_TRACE("- EventTimer::set_timerspec()");
}

void EventTimer::arm(const int fd, itimerspec const *timerspec) {
...
void EventTimer::disarm() const {
    ...
    arm(timerfd_, &timerspec);

    SPDLOG_TRACE("- EventTimer::disarm()");
}

void EventTimer::watch_fd(int fd, uint32_t events, struct timer_task *task) {
    ...
    ep.events = events;
    ep.data.ptr = task;
    epoll_ctl(evfd_, EPOLL_CTL_ADD, fd, &ep);
    watched_fd_++;
}

void EventTimer::watch_timerfd() {
    SPDLOG_TRACE("+ EventTimer::watch_timerfd()");
    task_.run = EventTimer::run;
    task_.data = reinterpret_cast<void *>(this);

    watch_fd(timerfd_, EPOLLIN, &task_);
    SPDLOG_TRACE("- EventTimer::watch_timerfd()");
}

void EventTimer::unwatch_fd(int fd) {
    if (evfd_ < 0) {
...
void EventTimer::wait_event() {
    ...
    const auto ready = epoll_wait(evfd_, ep, std::size(ep), 0);
...
void EventTimer::run(timer_task const *task, uint32_t events) {
    ...
    if (read(timer->timerfd_, &event, sizeof(event)) != sizeof(event)) {
        if (errno != EAGAIN)
            spdlog::critical("Failed to read timer. {}", strerror(errno));
        return;
    }

    timer->callback_(timer->callback_data_);
}
