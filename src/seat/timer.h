...
public:
    ...
    static uint32_t watched_fd_;
    static int evfd_;
    int timerfd_;
    struct itimerspec timerspec_{};
    struct timer_task task_{};
    evtimer_cb callback_;
    void *callback_data_;
...
private:
     ...
     * internal
     */
    static void arm(int fd, itimerspec const *timerspec);
     ...
     * internal
     */
    static void watch_fd(int fd, uint32_t events, struct timer_task *task);
