...
void Feedback::handle_sync_output(void *data, struct wp_presentation_feedback *wp_presentation_feedback,
    ...
    if (f->observer_) {
        f->observer_->notify_feedback_sync_output(f, wp_presentation_feedback, output);
    }
}

void Feedback::handle_presented(void *data,
    ...
    if (f->observer_) {
        f->observer_->notify_feedback_presented(f, wp_presentation_feedback,
                                                tv_sec_hi,
                                                tv_sec_lo,
                                                tv_nano_sec,
                                                refresh,
...
void Feedback::handle_discarded(void *data, struct wp_presentation_feedback *wp_presentation_feedback) {
    auto f = static_cast<Feedback *>(data);
    if (f->feedback_ != wp_presentation_feedback) {
        return;
    }
